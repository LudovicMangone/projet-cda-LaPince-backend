...
describe("[POST] /api/auth/register", () => {
  ...
  it("should return 422 if data is invalid", async () => {
    ...
    const response = await fetch(`${BASE_URL}/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "L", email: "notanemail", password: "weak" }),
    });
...
describe("[POST] /api/auth/login", () => {
  ...
  it("should return user and token on valid credentials (200)", async () => {
    ...
    await fetch(`${BASE_URL}/register`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Ludo", email: "ludo@lapince.fr", password: "Password123" }),
    });
    ...
    const response = await fetch(`${BASE_URL}/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: "ludo@lapince.fr", password: "Password123" }),
    });
    const body = await response.json();
  ...
  it("should return 401 if password is wrong", async () => {
    ...
    const response = await fetch(`${BASE_URL}/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: "ludo@lapince.fr", password: "WrongPassword1" }),
    });
  ...
  it("should return 401 if email does not exist", async () => {
    ...
    const response = await fetch(`${BASE_URL}/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email: "unknown@lapince.fr", password: "Password123" }),
    });
